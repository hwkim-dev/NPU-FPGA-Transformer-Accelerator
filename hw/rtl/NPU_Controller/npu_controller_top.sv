...
module npu_controller_top #() (
    ...
    // ===| Status Backflow : NPU -> CPU (read path) |===========================
    // Routed up from NPU_top's mmio_npu_stat aggregator. Continuous IN_valid
    // keeps the AXIL_STAT_OUT FIFO non-empty so the AXI4-Lite read handshake
    // can complete from the host side (s_arready = ~rvalid_r && ~fifo_empty).
